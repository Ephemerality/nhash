...
def compute_hash(buf, len):
    ...
    # Finalize the hash
    hash = TO128(hash + ((tail_size * 8) << 64))
    ...
    if B < Y:
        B = B + 0x101
    H = TO128(A * B)
    H = TO128((0x101 * TO64(H >> 64)) + TO64(H))
    H = TO128((0x101 * TO64(H >> 64)) + TO64(H))
    if H >> 64:
        H = H + 0x101
...
def hash_muladd(hash, mul, add):
    ...
    r2 = TO64(c2 + (c6 << 1) + a23)
    r3 = TO64(c3                   + HI(r2))
    r0 = TO64(c0 + (c4 << 1) + a0  + (r3 >> 31))
    r1 = TO64(c1 + (c5 << 1) + a1  + HI(r0))

    # Return as uint128_t
    result = (((r3 << 32) & 0x7fffffffffffffff) | LO(r2)) + HI(r1)
    return TO128((result << 64) | TO64((r1 << 32) | LO(r0)))
